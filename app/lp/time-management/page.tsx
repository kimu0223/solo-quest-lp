...
export default async function TimeManagementLP() {
  // A/Bテスト: バリアント取得
  const headersList = await headers();
  const cookieStore = await cookies();

  const variantFromHeader = headersList.get('x-ab-variant') as Variant | null;
  const variantFromCookie = cookieStore.get('ab_time_cta')?.value as Variant | undefined;
  const ctaVariant: Variant = variantFromHeader || variantFromCookie || 'a';

  const ctaText = AB_TESTS['time-cta'].variants[ctaVariant];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans text-slate-800">
        ...
        <section className="py-20 bg-teal-600 text-white text-center">
          <div className="max-w-3xl mx-auto px-6">
            ...
            <CTAButton
              ...
              eventLabel="time_management_lp_cta"
      ...
      <AbTestTracker testId="time-cta" variant={ctaVariant} pagePath="/lp/time-management" />
    </div>
  );
}